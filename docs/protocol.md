...
- **post-1.0 (stage 1 shipped)** &mdash; MIL-STD-2525 / APP-6 symbol renderer, in two stages. The schema's `SymbolIdentifier` field is wired through; the rendering lives on the map widget's side (`Kuestenlogik.Bowire.Map`), not in this plugin. **Stage 1 landed in Bowire 2.8 (`c2df2605`, 2026-09-14):** every pin whose entity carries a SIDC is drawn by [milsymbol](https://github.com/spatialillusions/milsymbol) (MIT, vendored next to MapLibre) &mdash; both the 15-letter 2525C and the 20-digit 2525D form &mdash; with the four affiliation shapes as the fallback while the library loads or for a code it cannot frame. This sample's thirteen tracks are the test data; a function id milsymbol has no icon for (`SFGPUCT`) still gets its frame. **Stage 2, open:** the multipoint tactical graphics milsymbol does not draw (lines, areas, arrows) &mdash; a port of [mil-sym-ts](https://github.com/missioncommand/mil-sym-ts) (Apache-2.0, MIL-STD-2525D+), weighed on 2026-09-15 and settled on vendoring the TypeScript in the browser: its `RenderSymbol2D` takes the viewport (pixel size and bounding box) and redraws arrowheads and the like on every zoom, so a server endpoint would be called per graphic per map move, and its label placement measures text through a canvas, which a .NET port would have to replace with a native font stack. The test data landed first: the sample's overlay carries seven 2525D control measures, one per `SymbolLocation` case the standard has a symbol for (line, polygon, multipoint, corridor, fan, ellipse), in the twenty-digit numeric form. The rendering itself is still open. Service-Bus / Artemis-flavoured AMQP 1.0 discovery (parallel item on the AMQP plugin's side) could similarly land as a vendor-specific follow-on.
