...
| Path | Purpose |
|---|---|
| **`__bowireMtls__`** (shared marker) | The preferred path, and the one the workbench's mTLS auth profile writes: client certificate and private key as PEM, the key's passphrase when it is encrypted, an optional CA certificate the server's chain is pinned to, and `allowSelfSigned`. Material that does not load &mdash; a wrong passphrase, a truncated paste &mdash; comes back as status `bad-transport-config` with the loader's reason, before any dial. It is the same marker the core REST / gRPC / Kafka / AMQP plugins read (`Kuestenlogik.Bowire.Auth.MtlsConfig`), so an mTLS profile set up once in the workbench works on TacticalAPI the same way it works everywhere else. When it is present, the legacy PFX keys below are ignored so two client certificates never load at once. |
...
- **v1.0.17 (2026-09-14)** &mdash; the four contract fixes: the `ResponseHeader` read on every reply and frame, `tacticalapi:refused` (#66); gRPC-Web on the `useGrpcWeb` setting and the shared transport marker, `allowSelfSignedCerts` finally consumed (#67); the contract's `Required:` fields and property notes reaching the invoke form (#68); the sample writing as well as reading, with the request bodies in its README (#69). `streamIdleSeconds` doing what it says, and `invocationDeadlineSeconds` unary-only. The upstream pin as a git ref (`TacticalApiProtoRef`), tag-ready.
- **v1.0.18 (2026-09-15)** &mdash; the `__bowireMtls__` marker read in full (passphrase, CA pinning, `bad-transport-config` for material that does not load); the mock emitter seeing refusals and replaying over the recorded transport; the sample refusing an incomplete blue force, honouring `expiry_time`, cloning every frame &mdash; and tested by its own project.
- **v1.0.19 (2026-09-16)** &mdash; `IBowireStreamingWithWireBytes` (recorded subscriptions replay 1:1), `_tacticalapi:objectCount` / `_tacticalapi:responseBytes` / `_tacticalapi:warning` on every unary result, `TacticalApiMockEmitter.Completion`; the sample's overlay of seven 2525D control measures, one per `SymbolLocation` case, as test data for the multipoint renderer.
- **v1.0.20 (2026-09-19)** &mdash; the four declared settings reach the metadata bag that reads them; the sample's tracks carry specific symbols and plausible positions, a second phase line in the 2525C string form; the released assembly carries the released version.
- **v1.1.0 (2026-09-19)** &mdash; the sample writes every one of the eleven object types, not only symbols: `SituationObjectMapper` walks the contract's update/served mirror once by reflection, an identity keeps its type for life, request bodies for a route, an action task and an overlay in the sample README &mdash; and every README body is sent by a test. **A note on the version:** by SemVer, 1.0.17 (gRPC-Web, a new status), 1.0.18 (new public API) and 1.0.19 (a new interface, three metadata keys &mdash; and the Bowire floor moving from 2.2.1 to 2.7.0, which the wire-bytes interface and the marker's certificate loader require) were minor releases tagged as patches; the cascade's auto-tag bumps patch and the manual tags followed it. Published packages are immutable, so those tags stand; this release takes the minor the line was owed. Hosts on Bowire below 2.7.0 stay on 1.0.16.
- **post-1.0 (both stages shipped)** &mdash; MIL-STD-2525 / APP-6 symbol renderer, in two stages. The schema's `SymbolIdentifier` field is wired through; the rendering lives on the map widget's side (`Kuestenlogik.Bowire.Map`), not in this plugin. **Stage 1 landed in Bowire 2.8 (`c2df2605`, 2026-09-14):** every pin whose entity carries a SIDC is drawn by [milsymbol](https://github.com/spatialillusions/milsymbol) (MIT, vendored next to MapLibre) &mdash; both the 15-letter 2525C and the 20-digit 2525D form &mdash; with the four affiliation shapes as the fallback while the library loads or for a code it cannot frame. This sample's thirteen tracks are the test data — each with a function id the library has an icon for, a patrol craft or a tank rather than a bare frame; a function id it has no icon for still gets its frame. **Stage 2 landed in Bowire 2.8 too (2026-09-16):** the multipoint tactical graphics milsymbol does not draw &mdash; lines, areas, arrows, corridors, fans, ellipses &mdash; are drawn by [mil-sym-ts](https://github.com/missioncommand/mil-sym-ts) (Apache-2.0, MIL-STD-2525D/E, APP-6D), vendored in the browser rather than ported to .NET: its `RenderSymbol2D` takes the viewport and redraws arrowheads on every zoom, so a server endpoint would have been called per graphic per map move, and its label placement measures text through a canvas a port would have had to replace with a native font stack. The widget reads a `SymbolLocation`'s `line` / `polygon` / `multipoint` / `corridor` points, an `ellipse`'s three points and a `fan`'s ranges and azimuths back into one geometry each, and the 2525D code from `NumericIdentifier`'s two ten-digit halves. A 2525C-coded graphic is translated to its 2525D entity through a crosswalk built from Esri's joint-military-symbology-xml legacy table before it is drawn. This sample's overlay of eight control measures — one per location case, plus a phase line in the 2525C string form — is the test data; `routeLocation` and `sketchLocation` have no 2525 graphic and are not drawn. The parallel item on the AMQP plugin's side &mdash; Service-Bus / Artemis-flavoured AMQP 1.0 discovery &mdash; landed there on 2026-09-17, by the same reasoning: a wire with no discovery of its own, two brokers that each answer a question about themselves, and a fallback for everything that does not.

## Sample

A canonical mini-server lives at [`Bowire.Protocol.TacticalApi/samples/Kuestenlogik.Bowire.Protocol.TacticalApi.Sample`](https://github.com/Kuestenlogik/Bowire.Protocol.TacticalApi/tree/main/samples/Kuestenlogik.Bowire.Protocol.TacticalApi.Sample) &mdash; RadarSweep grown into a *combined* sample that serves all three services and mounts the embedded workbench beside them. `dotnet run`, then open <http://localhost:5191/bowire> for the workbench (HTTP/1.1); the gRPC services listen on `http://localhost:5192` (cleartext HTTP/2) and are already seeded into the Sources rail, so an external Bowire connects with `bowire --url tacticalapi@http://localhost:5192`. The workbench port also serves the same three services as gRPC-Web, so the plugin's `useGrpcWeb` setting has something to dial (`tacticalapi@http://localhost:5191`). Thirteen `Situation` tracks in five groups and an overlay of eight control measures exercise the read side, and three writes are worth driving by hand, each with a request body in the sample's README that carries the envelope upstream's reference client sets: `Situation.AddOrUpdateSituationObjects` places a symbol, changes one property of it sparsely, and &mdash; with `reporter` left out &mdash; is refused the way a real server refuses; `OwnPose.UpdatePosition` moves the own pose, the blue force flagged `own_blue_force` and the UAV mounted on it in one call; a blue force added via `AddOrUpdateBlueForces` and then left alone comes back once with `is_deleted` after 30 s. The ports are split because ALPN lives in the TLS handshake, so one *cleartext* socket cannot serve both HTTP/1.1 and h2c. For a full Harbor Control Center scene, see the harbor-demo sibling [`Bowire.Samples/harbor-demo/src/Kuestenlogik.Bowire.Samples.TacticalApi`](https://github.com/Kuestenlogik/Bowire.Samples/tree/main/harbor-demo/src/Kuestenlogik.Bowire.Samples.TacticalApi).
