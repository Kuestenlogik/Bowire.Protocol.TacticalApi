...
- **post-1.0** &mdash; MIL-STD-2525 / APP-6 symbol renderer, in two stages. The schema's `SymbolIdentifier` field is already wired through; the rendering lands on the map widget's side (`Kuestenlogik.Bowire.Map`), not in this plugin. First with [milsymbol](https://github.com/spatialillusions/milsymbol) (MIT), which turns a single-point SIDC into the correct tactical-affiliation glyph; then an own .NET port of [mil-sym-ts](https://github.com/missioncommand/mil-sym-ts) (Apache-2.0, MIL-STD-2525D+) for the multipoint tactical graphics milsymbol does not draw. Service-Bus / Artemis-flavoured AMQP 1.0 discovery (parallel item on the AMQP plugin's side) could similarly land as a vendor-specific follow-on.
